...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val closeFilter = IntentFilter(OverlayService.ACTION_OVERLAY_CLOSE) ;LocalBroadcastManager.getInstance(this).registerReceiver(overlayCloseReceiver, closeFilter)
    }
    ...
    private val overlayDeathReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {// 如果 overlay UI flag 是開的，就重啟 Service// 重新推一次當前字幕給 overlay
    ...
    private val overlayCloseReceiver = object : BroadcastReceiver() {//overlay close 加 receiver 接廣播，呼叫 resetPlayback()
        override fun onReceive(context: Context?, intent: Intent?) { if (intent?.action == OverlayService.ACTION_OVERLAY_CLOSE) {  buttonLaunchOverlay.performClick()  }   }
    }
